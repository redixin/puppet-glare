...
#
# Configure the keystone_authtoken section in the Glare configuration file
#
# === Parameters
#
# [*username*]
...
class :glare::authtoken(
  ...
  if is_service_default($password) and $::glare::keystone_password == undef {
    fail('Please set password for Glare service user')
  }

  #NOTE(degorenko): Use pick to keep backward compatibility
  $username_real          = pick($::glare::keystone_user, $username)
  $password_real          = pick($::glare::keystone_password, $password)
  $project_name_real      = pick($::glare::keystone_tenant, $project_name)
  $auth_url_real          = pick($::glare::identity_uri, $auth_url)
  $auth_uri_real          = pick($::glare::auth_uri, $auth_uri)
  $memcached_servers_real = pick($::glare::memcached_servers, $memcached_servers)
  $token_cache_time_real  = pick($::glare::token_cache_time, $token_cache_time)
  $signing_dir_real       = pick($::glare::signing_dir, $signing_dir)
